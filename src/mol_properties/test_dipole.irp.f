...
implicit none 
  ...
  do istate = 1, N_states
   do jstate = 1, N_states
    print*,istate,jstate
    print*,matrix(jstate,istate) , multi_s_x_dipole_moment(jstate,istate) 
    accu += dabs(matrix(jstate,istate) - multi_s_x_dipole_moment(jstate,istate) )
   enddo
  enddo
  print*,'accu = ',accu
